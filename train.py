...
class Trainer:
    def __init__(self, batch_size: int, sub_division: int, image_size: int):
        ...
        self.loss_fn = YOLOv4Loss(num_class=self.num_class, yolo_score_threshold=self.yolo_score_threshold,
                                  label_smoothing_factor=self.label_smoothing_factor, use_focal_loss=True,
    ...
    def log_metrics(self, writer: tf.summary.SummaryWriter, dataset: tf.data.Dataset):
        ...
        loss, bboxes, scores, classes, valid_detections = self.validation(data['image'], data['label'])

        gt_boxes = data["bbox"]
        num_of_gt_boxes = data["num_of_bbox"]

        for frame in zip(bboxes.numpy(), classes.numpy(), scores.numpy(), valid_detections.numpy(), gt_boxes.numpy(), num_of_gt_boxes.numpy()):
        ...
        with writer.as_default():
            ...
            tf.summary.scalar('mean loss', loss.numpy() / (self.batch_size / self.sub_division), step=int(self.ckpt.step))
    ...
    def train_one_epoch(self):
        ...
        for data in self.dataset_train:
            ...
            if int(self.ckpt.step) % self.step_to_validate:
